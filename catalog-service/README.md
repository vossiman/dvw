...
unknown content is preserved byte-for-byte as custom configuration. The
effective file is then written atomically with custom directives first, because
OpenSSH uses the first value it obtains for each parameter.
...
or an unmodified effective document generated by this service. Requests with a
stale revision, edited managed markers, or metadata from a newer service schema
return `409` rather than silently losing or downgrading configuration.
