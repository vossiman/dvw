...
Window snapshots and the bulk status endpoint's container pick use the same
canonical-container resolver as attach/connect.
...
for operator data. Recognizing every version is not a downgrade path, though: a
block generated by a *newer* service is refused with `409` rather than stripped
and re-emitted at this service's version, which would roll back the SSH config
of every client that synced next. That check reads the version out of the marker
itself, so it holds even for a file copied in without its metadata. The effective
file is then written atomically with custom directives first, because OpenSSH
uses the first value it obtains for each parameter.
...
than silently losing or downgrading configuration.

The blocking file I/O runs off the event loop, serialized by a lock held *in the
worker thread*. That placement is deliberate: a disconnected client cancels the
handler task, which cannot stop a thread already mid-write, so a lock owned by
