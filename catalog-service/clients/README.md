...
| `catalog.sh` | the former file-based `catalog.sh` | every `catalog_*` call → REST; devpod-local helpers unchanged |
| `ssh-sync.sh` | the former file-based `ssh-sync.sh` | blueprint via `GET/PUT /v1/blueprint` |
...
Every function degrades gracefully when the service is unreachable: read paths
print a clear "service unreachable" message and return non-zero;
`_dvw_load_probe` marks all workspaces `unreachable`; the blueprint refresh is a
silent no-op.
