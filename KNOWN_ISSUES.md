...
- 🟢 **MITIGATED — `devpod up` on a running container can wipe `content/` and leave a stale bind mount** — Confirmed 2026-05-08 after losing uncommitted source on `financepdfs-git-main`. `devpod up` (verified with `--ide cursor`, suspected for other modes) on a workspace whose container is *already running* re-synthesizes the agent-side workspace dir on the provider host: `rm -rf content/` + sparse re-clone of just `.devcontainer/`, **without recreating the container**. The container's bind mount keeps pointing at the old `content/` inode, which is now an unlinked zombie kept alive only by the mount.
  ...
  **Three layers of defense in dvw:**
  ...
  2. **Provider-level safety wrapper for all cold-path `devpod up` callers.** `_dvw_safe_devpod_up` (in `devpod/lib/connect.sh`) wraps the three remaining call sites — `_connect_ssh`, `_connect_cursor` cold-path, `cmd_blueprint`. Before running `devpod up`, it asks the provider host directly via `docker ps -a --filter label=dev.containers.id=<uid>` whether a container for this workspace exists. If yes (i.e. our local SSH probe was wrong about the workspace being down — transient network, sshd restart, etc.), refuse: interactively a `gum confirm` for explicit override, non-interactively an outright error. Catches the case where the local probe gives a false-negative on a live container. Commit `6086b95`.
  3. **Visibility — verified status, not inferred.** `dvw status`, `dvw doctor`, and the picker now compute the running indicator from container peek (`_dvw_workspace_health`), not `devpod status` (which only reports docker-level Running/Stopped and would happily say "running" while the bind mount was dead). Stale workspaces show `⚠ stale` (red+bold) in the picker, a `[FAIL]` line in `dvw doctor` with the recreate hint, and a `[WARN]` line below the `dvw status` table. Commits `3ffc574`, `a9af7ea`.
  ...
  **Out of scope for the wrapper:** `cmd_recreate` is destructive by design (typed on purpose); `cmd_start` and `cmd_new` aren't wrapped — start could be on a stopped workspace where `devpod up` is exactly the intent, and new has no prior container. Manual `devpod up` invocations outside dvw can still hit this. Upstream root cause (why `devpod up` is destructive on a running container) was not investigated; the wrapper sidesteps the question rather than answering it.
